...
async def create_pedido(data: PedidoCreate):
    ...
    if data.cliente_id:
        ...
        # Adicionar pontos do clube se o pedido for do Cardápio
...
async def assign_entregador_to_pedido(pedido_id: str, entregador_id: str, current_user: User = Depends(get_current_user)):
    ...
    # Enviar notificação WhatsApp para o ENTREGADOR
    if entregador.get('telefone'):
        whatsapp_notifications.schedule_entregador_notification(pedido_id, entregador['telefone'])
...
async def chatbot_process_message(data: ChatbotProcessMessage):
    ...
    try:
        ...
        # REGRA: Texto responde com texto, Áudio responde com áudio
        # Mensagem de texto NÃO deve gerar áudio de resposta
        # A resposta de áudio só é gerada quando o cliente envia áudio (tratado acima)
        
        # Processar mensagem com IA (formato texto normal)
        response = await chatbot_ai.process_message(
            phone=data.phone,
            message=data.message,
            push_name=data.push_name or "",
            for_audio_response=False  # Mensagem de texto = resposta de texto
        )
        
        return {
            "success": True, 
            "response": response, 
            "response_audio_base64": None,  # Texto não gera áudio
            "bot_paused": False
        }
    except Exception as e:
...
async def update_bot_settings(data: BotSettingsUpdate, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if data.chatbot_voice is not None:
