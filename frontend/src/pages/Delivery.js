...
import { 
  ...
  Volume2, VolumeX, Undo2, Receipt, ChevronDown, Bell
} from "lucide-react";
    ...
    try {
      ...
      if (novosPedidosAceite.length > 0 && previousPedidosRef.current.length > 0) {
        ...
        for (const novoPedido of novosPedidosAceite) {
          ...
          imprimirPedidoAutomatico(novoPedido, settingsRes.data);
        }
      }
            ...
            {showPrint && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handlePrintPedido(pedido, e); }}>
                    <Printer className="w-4 h-4 mr-2" />
                    Imprimir Ambos
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handlePrint2Via(pedido, 'entrega', e); }}>
                    <Receipt className="w-4 h-4 mr-2 text-blue-500" />
                    2ª Via - Entrega
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handlePrint2Via(pedido, 'preparo', e); }}>
