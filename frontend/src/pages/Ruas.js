...
import { 
  Route as RouteIcon, Search, Edit, Trash2, Plus
} from "lucide-react";
...
export default function Ruas() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const ruasRes = await axios.get(`${API}/ruas`, getAuthHeader());
  ...
  const handleSalvar = async () => {
    ...
    setSaving(true);
    try {
      const data = { nome: formData.nome.trim() };
  ...
  const ruasFiltradas = ruas.filter(r => {
    ...
    return r.nome.toLowerCase().includes(searchTerm.toLowerCase());
  });
    ...
    <div className="p-6 space-y-6">
      ...
      {ruasFiltradas.length > 0 && (
        <p className="text-sm text-muted-foreground text-center">
          {ruasFiltradas.length} rua(s) cadastrada(s)
        </p>
      )}
